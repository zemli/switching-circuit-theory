...
int count;
#ifndef lint
static char rcsid[] UTIL_UNUSED = "$Id: main.c ECE1733$";
#endif
...
DdNode *perfOp(int op, DdNode *f, DdNode *g, DdManager *dd)
{
    if(AND == op)
        ...
        printf("Please enter a correct conmmand...\nexit...\n");
        exit(1);
    }
}
...
DdNode *APPLY(int op, DdNode *f, DdNode *g, DdManager *dd)
{
    ...
    {
        return perfOp(op, rf, rg, dd);count++;
    }
    else if(fIdx == gIdx)
    {
     ...
     char ** argv)
{
    ...
    int		ok=0;		/* overall return value from main() */
    ...
    /* Build the BDDs for the nodes of the second network. */
    if (option->verify == TRUE || option->second == TRUE ||
    ...
    if(!Bnet_bddDump(dd, net1, "BDD1.dot", 0, 0))
        printf("ERROR!! couldn't print BDD for Network 1\n");
    if(!Bnet_bddDump(dd, net2, "BDD2.dot", 0, 0))
        printf("ERROR!! couldn't print BDD for Network 2\n");
    
    DdNode *f = net1->nodes->dd; //a pointer to a DdNode object that is the root of the decision diagram
    DdNode *g = net2->nodes->dd;
    printf("Ddmanager has %ld nodes...\n", Cudd_ReadNodeCount(dd));
    
    clock_t time1 = clock();
    DdNode *h = APPLY(option->opt, f, g, dd);
    Cudd_Ref(h);
    clock_t time2 = clock();
    
    printf("Network 1 has %d inputs, %d outputs\n", net1->ninputs, net1->noutputs);
    printf("inputs:\n");
    for(i=0; i<net1->ninputs; i++)
    {
        printf("%s\t", net1->inputs[i]);
    }
    printf("\n");
    printf("outputs:\n");
    for(i=0; i<net1->noutputs; i++)
    {
        printf("%s\t", net1->outputs[i]);
    }
    printf("\n");
    
    int bnodeCount = 0;
    BnetNode *nextbnode = net1->nodes;
    while(nextbnode != NULL)
    {
        bnodeCount++;
        printf("Node of network1 index=%d 0x%x\n", nextbnode->var, nextbnode->dd->index);
        nextbnode = nextbnode->next;
    }
    printf("there are %d nodes in Network 1\n", bnodeCount);
    
    nextbnode = net1->nodes;
    while(nextbnode != NULL)
    {
        //if(nextbnode->var == net1->ninputs-1)
        if(nextbnode->var == 0)
            f=nextbnode->dd;
        nextbnode = nextbnode->next;
    }
    
    printf("Network 2 has %d inputs, %d outputs\n", net2->ninputs, net2->noutputs);
    printf("inputs:\n");
    for(i=0; i<net2->ninputs; i++)
    {
        printf("%s\t", net2->inputs[i]);
    }
    printf("\n");
    printf("outputs:\n");
    for(i=0; i<net2->noutputs; i++)
    {
        printf("%s\t", net2->outputs[i]);
    }
    printf("\n");
    bnodeCount = 0;
    nextbnode = net2->nodes;
    while(nextbnode != NULL)
    {
        bnodeCount++;
        printf("Node of network2 index=%d 0x%x\n", nextbnode->var, nextbnode->dd->index);
        nextbnode = nextbnode->next;
    }
    printf("there are %d nodes in Network 2\n", bnodeCount);
    ...
    BnetNode *curBnode;
    if(!st_lookup(net1->hash, net1->inputs[0], &curBnode))
    {
        printf("ERROR!! input 0 of Network 1 not found....\n");
    }
    else
        printf("input 0  of Network 1 has var index %x\n", curBnode->var);
    int nSupportSize = Cudd_SupportSize(dd, curBnode->dd);
    printf("input 0 of Network 1 has support size: %d\n", nSupportSize);
    
    if(!st_lookup(net2->hash, net2->inputs[0], &curBnode))
    {
        printf("ERROR!! input 0 of Network 2 not found....\n");
    }
    else
        printf("input 0  of Network 2 has var index %x\n", curBnode->var);
    nSupportSize = Cudd_SupportSize(dd, curBnode->dd);
    printf("input 0 of Network 2 has support size: %d\n", nSupportSize);
    
    
    printf("f is \n");
    Cudd_PrintDebug(dd, f, Cudd_DagSize(f), 3);
    printf("g is \n");
    Cudd_PrintDebug(dd, g, Cudd_DagSize(g), 3);
    printf("h is \n");
    Cudd_PrintDebug(dd, h, Cudd_DagSize(h), 3);
    FILE *fp = fopen("BDDh.dot", "w+");
    if(!Cudd_DumpDot(dd, 1, &h, NULL, NULL, fp))
        printf("ERROR!!! couldn't print h!!\n");
    fclose(fp);
    
    DdNode *one = Cudd_ReadOne(dd);
    Cudd_Ref(one);
    DdNode *zero = Cudd_Not(one);
    printf("one->index=%d, zero->index=%d\n", one->index, zero->index);
               ...
               NtrOptions * option)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        } else if (STRING_EQUAL(argv[i],"-apply")) {
            i++;
            if (STRING_EQUAL(argv[i],"and")) {
                option->opt = AND;
            } else if (STRING_EQUAL(argv[i],"or")) {
                option->opt = OR;
            } else if (STRING_EQUAL(argv[i],"xor")) {
                option->opt = XOR;
            } else if (STRING_EQUAL(argv[i],"nand")) {
                option->opt = NAND;
            } else if (STRING_EQUAL(argv[i],"nor")) {
                option->opt = NOR;
            } else if (STRING_EQUAL(argv[i],"xnor")) {
                option->opt = XNOR;
            } else {
                goto usage;
            }
